...
class _SellerDashboardScreenState extends State<SellerDashboardScreen>
  ...
  final TextEditingController _editStoreNameController = TextEditingController();
  ...
  final TextEditingController _editInstagramController = TextEditingController();
  ...
  final List<Map<String, dynamic>> _navItems = [
    ...
    {'icon': Icons.inventory_2_outlined, 'activeIcon': Icons.inventory_2, 'label': 'Products', 'index': 1},
    {'icon': Icons.shopping_bag_outlined, 'activeIcon': Icons.shopping_bag, 'label': 'Orders', 'index': 2},
    {'icon': Icons.chat_outlined, 'activeIcon': Icons.chat, 'label': 'Messages', 'index': 3},
  ];
  
  final List<Map<String, dynamic>> _bottomNavItems = [
    {'icon': Icons.home_outlined, 'activeIcon': Icons.home, 'label': 'Home'},
    {'icon': Icons.inventory_2_outlined, 'activeIcon': Icons.inventory_2, 'label': 'Products'},
    {'icon': Icons.shopping_bag_outlined, 'activeIcon': Icons.shopping_bag, 'label': 'Orders'},
    {'icon': Icons.chat_outlined, 'activeIcon': Icons.chat, 'label': 'Messages'},
    {'icon': Icons.notifications_none_outlined, 'activeIcon': Icons.notifications, 'label': 'Notifications'},
  ...
  void initState() {
    ...
    _deleteProfileDialogScaleAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: _deleteProfileDialogAnimationController,
        curve: Curves.easeOutBack,
      ),
    );
    _deleteProfileDialogFadeAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: _deleteProfileDialogAnimationController,
        curve: Curves.easeIn,
      ),
    ...
    _activeOrdersDialogScaleAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
    ...
    _activeOrdersDialogFadeAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(
        parent: _activeOrdersDialogAnimationController,
        curve: Curves.easeIn,
      ),
  ...
  bool _validateCNIC(String value) {
    ...
    if (value.trim().length != 13 || !RegExp(r'^[0-9]+$').hasMatch(value.trim())) {
  ...
  Future<void> _loadStoreInfo() async {
    ...
    try {
      ...
      if (result['success'] && result['data'] != null) {
        ...
        } else if (storeData.containsKey('logo_url') && storeData['logo_url'] != null) {
          logoPath = storeData['logo_url'].toString();
        } else if (storeData.containsKey('logo_path') && storeData['logo_path'] != null) {
          logoPath = storeData['logo_path'].toString();
        } else if (storeData.containsKey('store_logo') && storeData['store_logo'] != null) {
          logoPath = storeData['store_logo'].toString();
        } else if (storeData.containsKey('image') && storeData['image'] != null) {
          logoPath = storeData['image'].toString();
        } else if (storeData.containsKey('image_url') && storeData['image_url'] != null) {
          logoPath = storeData['image_url'].toString();
        } else if (storeData.containsKey('store_image') && storeData['store_image'] != null) {
  ...
  Future<void> _loadOrders() async {
    ...
    try {
      ...
      if (result['success']) {
        setState(() {
          final List<Map<String, dynamic>> rawOrders = List<Map<String, dynamic>>.from(result['data'] ?? []);
  ...
  Future<void> _loadProducts() async {
    ...
    try {
      ...
      final productsResult = await ApiService.getStoreProducts(storeId, role: 'seller');
      
      if (productsResult['success'] && productsResult['data'] != null) {
        final productsList = productsResult['data'] as List<dynamic>;

        final mappedProducts = productsList.map((p) => p as Map<String, dynamic>).toList();
    ...
    try {
      ...
      if (image != null) {
        ...
        if (result['success']) {
          ...
          _showSnackBar(result['message'] ?? 'Failed to update store logo', Colors.red);
        }
      }
    } catch (e) {
  ...
  bool _validateDescription() {
    ...
    final alphanumericCount = description.replaceAll(RegExp(r'[^a-zA-Z0-9]'), '').length;
  ...
  Future<void> _handleSaveProduct() async {
    ...
    try {
      ...
      if (result['success']) {
        ...
        final failMsg = _isEditing ? 'Failed to update product' : 'Failed to add product';
                              ...
                              Expanded(
                                child: OutlinedButton(
                                  ...
                                  style: OutlinedButton.styleFrom(
                                    side: BorderSide(color: lightGray.withOpacity(0.5), width: 1.5),
                              ...
                              Expanded(
                                child: ElevatedButton(
                                  ...
                                  child: Row(
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    children: [
                                      Icon(Icons.logout, color: whiteColor, size: 18),
  ...
  Future<void> _handleDeleteProfile() async {
    ...
    _showSnackBar('Checking for active orders...', Colors.blue);
    ...
    try {
      ...
      if (!result['success']) {
        _showSnackBar('Failed to verify order status. Please try again.', Colors.red);
        return;
      }
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'You cannot deactivate your profile while you have ${activeOrders.length} active order(s). Please complete or cancel them first.',
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  fontSize: isMobile ? 14 : 16,
                                  color: lightGray,
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton(
                                  onPressed: () {
                                    _activeOrdersDialogAnimationController.reverse().then((_) {
                                      Navigator.of(context).pop();
                                    });
                                  },
                                  style: ElevatedButton.styleFrom(
                            ...
                            Row(
                              ...
                              children: [
                                Expanded(
                                  child: OutlinedButton(
                                    onPressed: () {
                                      _deleteProfileDialogAnimationController.reverse();
                                      Navigator.of(context).pop();
                                    },
                                    style: OutlinedButton.styleFrom(
                                      side: BorderSide(color: lightGray.withOpacity(0.5), width: 1.5),
                                ...
                                Expanded(
                                  child: ElevatedButton(
                                    onPressed: () async {
                                      ...
                                      Navigator.of(context).pop();
                                      if (widget.userId == null) return;
                                      _showSnackBar('Deactivating profile...', Colors.orange);
                                      final result = await ApiService.deactivateSellerProfile(widget.userId!);
                                      if (result['success']) {
                                        Navigator.pushReplacement(
                                          context,
                                          MaterialPageRoute(
                                        ...
                                        _showSnackBar(result['message'] ?? 'Failed to deactivate profile', Colors.red);
                                      }
                                    },
                                    style: ElevatedButton.styleFrom(
  ...
  PreferredSizeWidget _buildMobileAppBar() {
    ...
    return AppBar(
      ...
      title: (_selectedTab == 1 && _productSubTab != 'add' || _selectedTab == 2 || _selectedTab == 3)
        ? Container(
      ...
      actions: [
        Row(
          ...
          children: [
            if (!isMobile) ...[
              Text(
                _userData?['name'] ?? _storeInfo?['store_name'] ?? widget.sellerName,
                style: TextStyle(
                  color: darkText,
                  fontSize: 14,
                  fontWeight: FontWeight.w600,
      ...
      if (await logosDirectory.exists()) {
        ...
        for (var file in files) {
          if (file is File) {
            ...
            if (widget.userId != null && fileBaseName.contains('logo_${widget.userId}_')) {
  ...
  Widget _buildStorePicture({required double size, required Color iconColor, required Color backgroundColor}) {
    if (_storeLogoPath != null && _storeLogoPath!.isNotEmpty) {
      
      final isUrl = _storeLogoPath!.startsWith('http://') || _storeLogoPath!.startsWith('https://');
                ...
                ExpansionTile(
                  ...
                  children: [
                    ...
                    _buildDrawerNavItem(
                      ...
                      index: 1,
                      isSubItem: true,
                      isSelectedOverride: _productSubTab == 'add' && !_isEditing,
                       onTapOverride: () {
                         Navigator.of(context).pop();
                        _onTabChanged(1);
          ...
          children: List.generate(_bottomNavItems.length, (index) {
            ...
            return Expanded(
              child: GestureDetector(
                onTap: () {
                  if (index == _notificationsTabIndex) return; 
                  _onTabChanged(index);
                },
                  ...
                  ? Row(
                      ...
                      children: [
                        Icon(
                          ...
                          color: (isSelected || hasSelectedSubItem) ? brownColor : lightGray,
                          size: 22,
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Text(
                            ...
                            style: TextStyle(
                              fontSize: 15,
                              color: (isSelected || hasSelectedSubItem) ? brownColor : darkText,
  ...
  Widget _buildSubNavItem(String title, bool isSelected, {required VoidCallback onTap, IconData? icon}) {
    return MouseRegion(
      cursor: SystemMouseCursors.click,
      child: InkWell(
        onTap: onTap,
          ...
          child: _isSidebarHovered
              ? Row(
                  ...
                  children: [
                    Icon(
                      isSelected ? activeIcon : icon,
                      color: isLogout ? Colors.red : (isSelected ? brownColor : lightGray),
                      size: 22,
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Text(
                        ...
                        style: TextStyle(
                          fontSize: 15,
                          color: isLogout ? Colors.red : (isSelected ? brownColor : darkText),
                          fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500,
                          letterSpacing: -0.2,
                        ),
                        overflow: TextOverflow.ellipsis,
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: isMobile ? 2 : 3,
            child: (_selectedTab == 1 && _productSubTab != 'add' || _selectedTab == 2)
              ? Center(
                  child: Container(
                    constraints: BoxConstraints(
                      maxWidth: isMobile ? double.infinity : 500,
  ...
  Widget _buildDashboardView() {
    ...
    final filteredRecentOrders = _orders.where((order) {
      ...
      return id.contains(query) || status.contains(query) || customer.contains(query);
    }).toList();
          ...
          child: Column(
            ...
            children: [
              ...
              if (filteredRecentOrders.isEmpty)
                ...
                Column(
                  children: filteredRecentOrders.take(5).map((order) => Padding(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          _storeInfo?['bio'] ?? 'Growing my fashion business with FitAura.',
                          style: TextStyle(
                            fontSize: isMobile ? 14 : 16,
  ...
  Widget _buildProfileStats(bool isMobile) {
    ...
    final ordersCount = _orders.where((o) {
      final status = (o['status'] ?? o['order_status'] ?? '').toString().toLowerCase();
    ...
    return Container(
      ...
      child: Row(
        children: [
          _buildStatItem('Total Products', productsCount.toString(), Icons.shopping_bag_outlined),
          _buildStatVerticalDivider(),
          _buildStatItem('Overall Rating', rating, Icons.star_rounded, isRating: true),
          _buildStatVerticalDivider(),
          _buildStatItem('Orders Completed', ordersCount.toString(), Icons.check_circle_outline_rounded),
        ],
      ),
    );
  }

  Widget _buildStatItem(String label, String value, IconData icon, {bool isRating = false}) {
    return Expanded(
      child: Column(
        children: [
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          _buildInfoTile('Store URL', _storeInfo?['website'] ?? 'fitaura.com/store/unique', Icons.link_rounded),
          _buildInfoTile('Instagram', _storeInfo?['instagram'] ?? '@fitaura_official', Icons.camera_alt_outlined),
          _buildInfoTile('Category', _storeInfo?['category'] ?? 'Fashion & Apparel', Icons.category_outlined),
          _buildInfoTile('Member Since', _formatDate(_storeInfo?['created_at']), Icons.calendar_today_outlined),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          _buildInfoTile('Full Name', _userData?['name'] ?? widget.sellerName, Icons.badge_outlined),
          _buildInfoTile('Email Address', _userData?['email'] ?? 'seller@fitaura.com', Icons.email_outlined),
          _buildInfoTile('Contact', _userData?['contact_number'] ?? '+92 300 1234567', Icons.phone_android_outlined),
          _buildInfoTile('CNIC Number', _obfuscateCNIC(_userData?['cnic']), Icons.fingerprint_rounded),
          _buildInfoTile('Office Address', _userData?['address'] ?? 'Street 1, Lahore, Pakistan', Icons.location_on_outlined),
        ],
      ),
    );
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  value,
                  style: SellerDashboardStyles.infoValueStyle,
                ),
              ],
            ),
          ),
  ...
  void _showEditStoreDialog() {
    ...
    _showProfileEditDialog(
      ...
      onSave: (setDialogState) async {
        ...
        final isValid = _validateStoreName(_editStoreNameController.text) & 
        ...
        if (result['success']) {
          ...
          _showSnackBar(result['message'] ?? 'Failed to update store', Colors.red);
        }
      },
    );
  }
      ...
      children: [
        TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
              borderSide: errorText != null ? const BorderSide(color: Colors.red, width: 2.0) : BorderSide(color: brownColor, width: 2.0),
            ),
  ...
  Widget _buildOrdersView() {
    ...
    if (filteredOrders.isEmpty) {
      ...
      if (_orderSubTab == 'pending') message = 'No pending orders';
      else if (_orderSubTab == 'accepted') message = 'No accepted orders';
      else if (_orderSubTab == 'completed') message = 'No completed orders';
      else if (_orderSubTab == 'rejected') message = 'No rejected orders';
  ...
  Widget _buildProductsView() {
    ...
    if (_productSubTab == 'approved') {
      ...
      filteredProducts = _products.where((p) {
        final isVerified = p['is_verified'];
        final matches = isVerified == 1 || isVerified == '1' || isVerified == true;
        return matches;
      }).toList();
      ...
      filteredProducts = _products.where((p) {
        ...
        final matches = isVerified == 0 || isVerified == '0' || isVerified == false || isVerified == null;
        return matches;
      }).toList();
  ...
  Widget _buildMessagesView() {
    ...
    final filteredChats = query.isEmpty
        ? _chats
        : _chats.where((chat) {
            final otherUserName = (chat['other_user_name'] ?? '').toString().toLowerCase();
  ...
  Widget _buildChatCard(dynamic chat) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(color: isRead ? lightGray.withOpacity(0.1) : brownColor.withOpacity(0.2)),
      ),
      child: ListTile(
        ...
        title: Row(
          ...
          children: [
            ...
            Text(
              timeAgo,
              style: TextStyle(color: isRead ? lightGray : darkText.withOpacity(0.5), fontSize: 12),
            ),
          ],
        ),
        subtitle: Padding(
  ...
  Widget _buildChatAvatar({
    ...
    if (avatar != null && avatar.isNotEmpty) {
      try {
        ...
        return CircleAvatar(
          backgroundColor: isRead ? brownColor.withOpacity(0.1) : brownColor.withOpacity(0.2),
          radius: 24,
      ...
      child: GestureDetector(
        onTap: () async {
          ...
          if (!isRead && notificationId != null) {
            final result = await ApiService.markUserNotificationRead(notificationId is int ? notificationId : int.parse(notificationId.toString()));
            if (result['success']) {
              _loadNotifications();
            }
          }
          ...
          } else if (lowerTitle.contains('product') || lowerMessage.contains('product')) {
            if (lowerTitle.contains('approved') || lowerMessage.contains('approved')) {
              setState(() {
                _selectedTab = 1;
                _productSubTab = 'approved';
                _isProductsExpanded = true;
              });
            } else if (lowerTitle.contains('rejected') || lowerMessage.contains('rejected')) {
              setState(() {
                _selectedTab = 1;
                _productSubTab = 'rejected';
                _isProductsExpanded = true;
              });
            } else if (lowerTitle.contains('pending') || lowerMessage.contains('pending')) {
              setState(() {
                _selectedTab = 1;
                _productSubTab = 'pending';
                _isProductsExpanded = true;
                  ...
                  if (!isRead)
                    Positioned(
                      ...
                      child: Container(
                        width: 14,
                        height: 14,
                        decoration: SellerDashboardStyles.unreadIndicatorDecoration,
                      ),
                    ),
                ],
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      message,
                      style: SellerDashboardStyles.notificationMessageStyle(isRead),
                    ),
                  ],
                ),
              ),
                  ...
                  IconButton(
                    ...
                    onPressed: () async {
                      if (notificationId != null) {
                        try {
                          final result = await ApiService.deleteUserNotification(notificationId is int ? notificationId : int.parse(notificationId.toString()));
                          if (result['success']) {
                            _loadNotifications();
                            _showSnackBar('Notification deleted', Colors.black87);
                          } else {
                            _showSnackBar(result['message'] ?? 'Failed to delete', Colors.red);
                          }
                        } catch (e) {
              ...
              ? Column(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: _currentStep == 1 ? _buildVariantsForm(isMobile) : _buildProductFormContent(isMobile),
                    ),
                    SizedBox(height: 20),
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
                  borderSide: BorderSide(
                    color: error != null ? Colors.red : lightGray.withOpacity(0.3),
                  ),
            ),
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
        ...
        if (error != null)
          Positioned(
            right: 0,
            top: -8,
            child: _buildErrorTooltip(error),
        ),
      ],
    );
  }
  ...
  Widget _buildProductFormContent(bool isMobile) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          _isEditing ? 'Modify the product\'s information' : 'Manage the product\'s basic information',
          style: TextStyle(
            fontSize: isMobile ? 13 : 14,
            color: lightGray,
          ),
        ...
        if (isMobile) ...[
           ...
           _buildDropdownField(
             ...
             items: ['Tops', 'Bottoms', 'Dresses', 'Sets', 'Activewear', 'Sleepwear'],
             onChanged: (value) {
               setState(() {
        ...
        TextField(
          ...
          decoration: InputDecoration(
            ...
            border: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
                  borderSide: BorderSide(
                    color: _descriptionError != null ? Colors.red : lightGray.withOpacity(0.3),
                  ),
            ),
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(12),
  ...
  void _showSnackBar(String message, Color backgroundColor) {
    bool isError = backgroundColor == Colors.red || 
      ...
      children: [
        ...
        if (isMobile) ...[
          ...
          _buildDropdownField(
            ...
            items: ['Red', 'Blue', 'Green', 'Black', 'White', 'Yellow', 'Pink', 'Purple', 'Grey', 'Brown', 'Orange', 'Navy', 'Maroon', 'Teal'],
            onChanged: (val) {
              setState(() {
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text('Price: ${variant['price_per_variant']} | Stock: ${variant['stock_quantity']}'),
                  trailing: IconButton(
            ...
            Row(
              ...
              children: [
                OutlinedButton.icon(
                  ...
                  icon: Icon(Icons.description_outlined, size: 16, color: brownColor),
                  label: Text('Review', style: TextStyle(color: brownColor, fontSize: 12)),
                  style: OutlinedButton.styleFrom(
  ...
  void _showOrderDetails(Map<String, dynamic> order) async {
    ...
    if (!result['success']) {
      _showSnackBar(result['message'] ?? 'Failed to load order details', Colors.red);
      return;
    }
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Flex(
                                  ...
                                  children: [
                                    if (isMobile)
                                      _buildDetailRow(Icons.person_outline, 'Customer', orderData['customer_name'] ?? 'N/A')
                                    else
                                      Expanded(child: _buildDetailRow(Icons.person_outline, 'Customer', orderData['customer_name'] ?? 'N/A')),
                                      
                                    if (isMobile) const SizedBox(height: 12) else const SizedBox(width: 12),
                                    
                                    if (isMobile)
                                      _buildDetailRow(Icons.email_outlined, 'Email', orderData['customer_email'] ?? 'N/A')
                                    else
                                      Expanded(child: _buildDetailRow(Icons.email_outlined, 'Email', orderData['customer_email'] ?? 'N/A')),
                                  ],
                                ),
                                const SizedBox(height: 12),
                                Flex(
                                  ...
                                  children: [
                                    if (isMobile)
                                      _buildDetailRow(Icons.phone_outlined, 'Contact Number', orderData['contact_number'] ?? orderData['customer_phone'] ?? 'N/A')
                                    else
                                      Expanded(child: _buildDetailRow(Icons.phone_outlined, 'Contact Number', orderData['contact_number'] ?? orderData['customer_phone'] ?? 'N/A')),
                                      
                                    if (isMobile) const SizedBox(height: 12) else const SizedBox(width: 12),
                                    
                                    if (isMobile)
                                      _buildDetailRow(Icons.payment_outlined, 'Payment Method', orderData['payment_method'] ?? 'N/A')
                                    else
                                      Expanded(child: _buildDetailRow(Icons.payment_outlined, 'Payment Method', orderData['payment_method'] ?? 'N/A')),
                                  ],
                                ),
                                const SizedBox(height: 12),
                                _buildDetailRow(Icons.location_on_outlined, 'Address', orderData['shipping_address'] ?? 'N/A', isMultiline: true),

                                const SizedBox(height: 24),
                                
                                _buildSectionTitle('Order Items', Icons.inventory_2_outlined),
                                const SizedBox(height: 12),
                                Container(
                                  decoration: BoxDecoration(
                                    color: beigeBackground,
                                    border: Border.all(color: lightGray.withOpacity(0.1)),
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                  child: Column(
                                    children: items.asMap().entries.map((entry) {
                                      ...
                                      if (item['image'] != null) {
                                        imageWidget = ClipRRect(
                                          borderRadius: BorderRadius.circular(8),
                                          child: _buildImageWidget(item['image'], fit: BoxFit.cover),
                                        );
                                      } else {
                                        imageWidget = Icon(Icons.image, size: 24, color: lightGray);
                                      }

                                      return Container(
                                        decoration: BoxDecoration(
                                          border: isLast ? null : Border(
                                            bottom: BorderSide(color: lightGray.withOpacity(0.1)),
                                          ),
                                        ),
                                        padding: const EdgeInsets.all(12),
                                        child: Row(
                                          children: [
                                                  ...
                                                  children: [
                                                    ...
                                                    Expanded(
                                                      child: Column(
                                                        crossAxisAlignment: CrossAxisAlignment.start,
                                                        children: [
                                                          Text('Product', style: TextStyle(fontSize: 11, color: lightGray)),
                                                          const SizedBox(height: 2),
                                                          Text(
                                            ...
                                            Expanded(
                                              ...
                                              child: Column(
                                                crossAxisAlignment: CrossAxisAlignment.start,
                                                children: [
                                                  Text('Variant', style: TextStyle(fontSize: 11, color: lightGray)),
                                                  const SizedBox(height: 2),
                                                  Text(
                                            ...
                                            Expanded(
                                              ...
                                              child: Column(
                                                crossAxisAlignment: CrossAxisAlignment.center,
                                                children: [
                                                  Text('Qty', style: TextStyle(fontSize: 11, color: lightGray)),
                                                  const SizedBox(height: 2),
                                                  Text(
                                                    '${item['quantity']}',
                                                    style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: darkText),
                                            ...
                                            Expanded(
                                              ...
                                              child: Column(
                                                crossAxisAlignment: CrossAxisAlignment.end,
                                                children: [
                                                  Text('Price', style: TextStyle(fontSize: 11, color: lightGray)),
                                                  const SizedBox(height: 2),
                                                  Text(
                                                    'Rs ${item['price']}',
                                                    style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: brownColor),
                                                  ),
                                                ],
                                              ),
                                            ),
                                ...
                                _buildSectionTitle('Price Consideration', Icons.attach_money),
                                const SizedBox(height: 12),
                                _buildDetailRow(Icons.account_balance_wallet_outlined, 'Your Order Price', 'Rs $orderTotal'),

                                const SizedBox(height: 24),

                                _buildSectionTitle('Order Status', Icons.info_outline),
                                const SizedBox(height: 12),
                                _buildStatusChip('Current Status', orderData['order_status'] ?? 'Pending'),

                                const SizedBox(height: 24),

                                if (['pending', 'accepted', 'shipped', 'delivered'].contains(orderData['order_status']?.toString().toLowerCase())) ...[
                                  _buildSectionTitle('Actions', Icons.settings_outlined),
                                  const SizedBox(height: 16),
                                  Row(
                                    children: [
                                      if (orderData['order_status']?.toString().toLowerCase() == 'pending') ...[
                                        ElevatedButton.icon(
                                          onPressed: () {
                                            _updateOrderStatus(orderId, 'accepted');
                                            Navigator.of(context).pop();
                                          },
                                          icon: Icon(Icons.check_circle_outline, color: whiteColor, size: 20),
                                          label: Text(
                                            'Accept',
                                        ...
                                        ElevatedButton.icon(
                                          ...
                                          icon: Icon(Icons.cancel_outlined, color: whiteColor, size: 20),
                                          label: Text(
                                            'Reject',
                                            style: TextStyle(color: whiteColor, fontSize: 16, fontWeight: FontWeight.w600),
                                          ),
                                      ...
                                      ] else if (orderData['order_status']?.toString().toLowerCase() == 'accepted') ...[
                                        ElevatedButton.icon(
                                          onPressed: () {
                                            _updateOrderStatus(orderId, 'shipped');
                                            Navigator.of(context).pop();
                                          },
                                          icon: Icon(Icons.local_shipping_outlined, color: whiteColor, size: 20),
                                          label: Text(
                                            'Mark as Shipped',
                                            style: TextStyle(color: whiteColor, fontSize: 16, fontWeight: FontWeight.w600),
                                          ),
                                      ...
                                      ] else if (orderData['order_status']?.toString().toLowerCase() == 'shipped') ...[
                                        ElevatedButton.icon(
                                          onPressed: () {
                                            _updateOrderStatus(orderId, 'delivered');
                                            Navigator.of(context).pop();
                                          },
                                          icon: Icon(Icons.home_outlined, color: whiteColor, size: 20),
                                          label: Text(
                                            'Mark as Delivered',
                                            style: TextStyle(color: whiteColor, fontSize: 16, fontWeight: FontWeight.w600),
                                          ),
                                      ...
                                      ] else if (orderData['order_status']?.toString().toLowerCase() == 'delivered') ...[
                                        ElevatedButton.icon(
                                          onPressed: () {
                                            _updateOrderStatus(orderId, 'completed');
                                            Navigator.of(context).pop();
                                          },
                                          icon: Icon(Icons.done_all_outlined, color: whiteColor, size: 20),
                                          label: Text(
                                            'Mark as Completed',
                                            style: TextStyle(color: whiteColor, fontSize: 16, fontWeight: FontWeight.w600),
                                          ),
  ...
  Future<void> _updateOrderStatus(int orderId, String status) async {
    ...
    try {
      ...
      if (result['success']) {
        ...
        _showSnackBar(result['message'] ?? 'Failed to update order', Colors.red);
      }
    } catch (e) {
      _showSnackBar('An error occurred', Colors.red);
    } finally {
  ...
  Widget _buildProductCard({required Map<String, dynamic> product}) {
    ...
    final productName = product['product_name'] ?? product['name'] ?? 'Product Name';
    ...
    if (product['product_images'] != null) {
      try {
        if (product['product_images'] is String) {
          ...
          final imagesList = jsonDecode(product['product_images']) as List<dynamic>;
    ...
    productImage ??= product['image'] ?? product['image_url'] ?? product['images']?[0];
    
    final productNumber = product['product_number'] ?? productId.toString().padLeft(2, '0');
  ...
  Widget _buildStatusChip(String label, String status) {
    ...
    if (isApproved) {
      if (status == '1' || status.toLowerCase() == 'verified') displayStatus = 'Approved';
      else if (status.toLowerCase() == 'accepted') displayStatus = 'Accepted';
      else if (status.toLowerCase() == 'delivered') displayStatus = 'Delivered';
      else if (status.toLowerCase() == 'completed') displayStatus = 'Completed';
    } else if (isRejected) {
  ...
  void _showProductDetails(String productId) {
    ...
    ApiService.getProductById(productId).then((result) {
      ...
      if (result['success'] && result['data'] != null) {
                              ...
                              child: SingleChildScrollView(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (product['product_images'] != null) ...[
                                      _buildSectionTitle('Product Images', Icons.image_outlined),
                                      const SizedBox(height: 12),
                                      SizedBox(
                                        height: 120,
                                        child: ListView(
                                          scrollDirection: Axis.horizontal,
                                          children: _buildProductImagesList(product['product_images']),
                                        ),
                                      ),
                                      const SizedBox(height: 24),
                                    ],
                                      ...
                                      children: [
                                        ...
                                        if (isMobile)
                                          _buildDetailRow(
                                            ...
                                            'Gender',
                                            product['gender'] != null && product['gender'].toString().isNotEmpty
                                          ...
                                          Expanded(
                                            child: _buildDetailRow(
                                              Icons.person_outline,
                                              'Gender',
                                              product['gender']?.toString().toUpperCase() ?? 'N/A',
                                            ),
                                          ),
                                      ],
                                    ),
                                        ...
                                        builder: (context, snapshot) {
                                          if (snapshot.connectionState == ConnectionState.waiting) {
                                            return Padding(
                                              ...
                                              child: Center(child: CircularProgressIndicator(color: brownColor)),
                                            );
                                          }
                                          ...
                                          if (snapshot.hasError || !snapshot.hasData || snapshot.data!.isEmpty) {
                                            return Text(
                                              ...
                                              style: TextStyle(color: lightGray, fontStyle: FontStyle.italic),
                                            );
                                          }
                                    ...
                                    const SizedBox(height: 24),
                                    if (product['is_verified'] == 1 || product['is_verified'] == '1' ||
                                        ...
                                        child: Row(
                                          children: [
                                            ...
                                            ElevatedButton.icon(
                                              ...
                                              icon: Icon(Icons.delete_outline, color: whiteColor, size: 20),
                                              label: Text(
                                                'Delete',
                                                style: TextStyle(
                                                  color: whiteColor,
        ...
        _showSnackBar(result['message'] ?? 'Failed to load product details', Colors.red);
      }
    });
  }
  ...
  Future<void> _handleDeleteProduct(Map<String, dynamic> product) async {
    ...
    try {
      final result = await ApiService.deleteProduct(int.parse(productId.toString()));
      ...
      if (result['success']) {
        ...
        _showSnackBar(result['message'] ?? 'Failed to delete product', Colors.red);
      }
    } catch (e) {
      _showSnackBar('Error: ${e.toString()}', Colors.red);
    } finally {
